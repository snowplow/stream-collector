...
public final class InstanceMetadataServiceResourceFetcher extends EC2ResourceFetcher {
    ...
    private String getToken() {
        ...
        LOG.info("uri: " +  URI.create(host + EC2_TOKEN_ROOT));
        LOG.info("headers: " +  TOKEN_TTL_HEADER + ":" + DEFAULT_TOKEN_TTL);
    ...
    private void handleException(SdkClientException e) {
        LOG.warn("Handling token exception ", e);
    ...
    private boolean isTokenUnsupported(SdkClientException sdkClientException) {
        LOG.info("message:" + sdkClientException.getMessage());
        if (sdkClientException instanceof AmazonServiceException) {
            LOG.info("error is instance of AmazonServiceException");
            AmazonServiceException serviceException = (AmazonServiceException) sdkClientException;
            LOG.info("status code " + serviceException.getStatusCode());
            LOG.info("cause" + serviceException.getCause().toString());
            LOG.info("isRetryable " + isRetryable(serviceException.getCause()));
        ...
        if (sdkClientException.getCause() instanceof SocketTimeoutException) {
            LOG.info("instance of SocketTimeoutException");
        }
