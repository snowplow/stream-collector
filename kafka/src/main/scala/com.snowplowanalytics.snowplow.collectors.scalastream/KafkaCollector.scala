...
import com.snowplowanalytics.snowplow.streams.kafka.KafkaFactory
import com.snowplowanalytics.snowplow.collector.core.{App, Config, Sinks, Telemetry}
import com.snowplowanalytics.snowplow.collectors.scalastream.sinks.{KafkaSink, KafkaSinkConfig}
...
object KafkaCollector extends App[KafkaSinkConfig](BuildInfo) {
  ...
  override def mkSinks(config: Config.Streams[KafkaSinkConfig]): Resource[IO, Sinks[IO]] =
    for {
      factory <- KafkaFactory.resource[IO]
      good    <- KafkaSink.create[IO](config.good, factory)
      bad     <- KafkaSink.create[IO](config.bad, factory)
