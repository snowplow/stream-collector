...
class PrometheusMetricsService(metricsConfig: PrometheusMetricsConfig, scalaVersion: String, appVersion: String)
  ...
  private val requestDurationHistogramBuilder =
    Histogram.build(HttpRequestDuration, HttpRequestDurationHelp).labelNames(Labels: _*)
  private val requestDurationHistogram: Histogram =
    metricsConfig
      .durationBucketsInSeconds
  ...
  private val applicationVersionGauge = Gauge
    .build("service_version", "Java, scala versions and collector version")
